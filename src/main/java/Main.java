...
public abstract class Main {
    ...
    public static List<List<Integer>> negativeRatings;
    ...
    public static void ReadRatings_HoldOneOut(String ratingFile) throws IOException {
        ...
        long startTime = System.currentTimeMillis();
        ArrayList<ArrayList<Rating>> user_ratings = new ArrayList<>();
        BufferedReader reader = new BufferedReader(
                new InputStreamReader(new FileInputStream(ratingFile)));
        String line;
        while ((line = reader.readLine()) != null) {
    ...
    public static void readSplitedRating(String trainingFile, String testingFile, String negativeFile) throws FileNotFoundException {
        List<Rating> trainRatings = getRatingFiles(trainingFile);
        testRatings = getRatingFiles(testingFile);

        int userCount = 0;
        int itemCount = 0;

        List<Rating> total = new ArrayList<>();
        total.addAll(trainRatings);
        total.addAll(testRatings);

        for (Rating rating : total) {
            if (rating.userId > userCount) {
                userCount = rating.userId;
            }

            if (rating.itemId > itemCount) {
                itemCount = rating.itemId;
            }
        }
        ...
        negativeRatings = new ArrayList<>();
        Scanner scanner = new Scanner(new File(negativeFile));
        ...
        while (scanner.hasNextLine()) {
            ...
            for (int i = 1; i < params.length; i++) {
                items.add(Integer.parseInt(params[i]));
            }

            negativeRatings.add(items);
        }
    ...
    private static ArrayList<Rating> getRatingFiles(String input) throws FileNotFoundException {
        BufferedReader reader = new BufferedReader(
